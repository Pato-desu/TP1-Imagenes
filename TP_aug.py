...
def renglones_separados(imagen):
    # --- Cargo imagen ------------------------------------------------------------
    ...
    # plt.imshow(img_bw, cmap='gray'), plt.show(block=False)


    img_zeros = img_bw==255
    ...
    #PARA PREGUNTAS 
    # SERIA FX, CX    a   FX, CX+1
    #       FX+1, CX  a   FX+1, CX+1

    #PARA EL RESTO
    # SERIA FX, CX     a  FX, CX+2
    #       FX+1, CX   a  FX+1, CX+2


    aux_columna = list(enumerate(suma_por_columna))
    column_high = sorted(aux_columna, key = lambda x: x[1], reverse=False)
    ...
    row_high = sorted(aux_fila, key = lambda x: x[1], reverse=False)
    ...
    #plt.imshow(p2_si, cmap ="gray"), plt.show(block = False)
